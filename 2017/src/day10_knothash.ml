...
module Knot : sig
  ...
  let create length =
    let range_list min max = 
      let rec range_list' list num = 
        if num < min then list
        else
          range_list' (num :: list) (num - 1) in
      range_list' [] max in
    
    if length < 2 then failwith "invalid length"
    else 
      { array = Array.of_list (range_list 0 (length - 1))
      ; position = 0
      ; skip_size = 0 }


  let reverse_subarray array start_index length =
    let array_length = Array.length array in
    for i = 0 to (length / 2) - 1 do
      let first_index = (start_index + i) % array_length in
      let last_index = (start_index + length - i - 1) % array_length in
      Array.swap array first_index last_index
    done
  
  
  let tie_knot t length =
    reverse_subarray t.array t.position length;
    { t with position = (t.position + t.skip_size + length) % (Array.length t.array)
           ; skip_size = t.skip_size + 1 }
    
  
  let hash t =
    assert (Array.length t.array >= 2);
    t.array.(0) * t.array.(1)
end
