...
let checksum rows =
  (* Returns a tuple of a new min and max given a current min, current max, and
     a value x *)
  let min_max (current_min, current_max) x =
    (min current_min x, max current_max x) in

  (* Finds the min and max in a list *)
  let min_max_list list =
    match list with
    | [] -> (0, 0)
    | head :: tail -> List.fold ~init:(head, head) ~f:min_max tail in

  (* Sums each row’s max - min *)
  List.map ~f:min_max_list rows
  |> List.fold ~init:0 ~f:(fun sum (min, max) -> sum + max - min)
...
let () =
  printf "checksum test_data_small = %d (should be 18)\n" (checksum test_data_small);
  printf "checksum test_data_large = %d (should be 32020)\n" (checksum test_data_large)
