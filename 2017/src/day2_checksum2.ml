...
let checksum rows =
  let rec find_cofactors row =
    let is_cofactor x y =  x mod y = 0 || y mod x = 0 in
    match row with
    | [] | [_] -> None
    | head :: tail ->
        match List.find tail ~f:(is_cofactor head) with
        | None -> find_cofactors tail
        | Some cofactor -> Some (head, cofactor) in
  let row_checksum row =
    match find_cofactors row with
    | None -> 0
    | Some (x, y) -> if x < y then y / x else x / y in
  List.map ~f:row_checksum rows
  |> List.fold ~init:0 ~f:(+)
...
let () =
  printf "checksum test_data_small = %d (should be 9)\n" (checksum test_data_small);
  printf "checksum test_data_large = %d (should be 236)\n" (checksum test_data_large)
